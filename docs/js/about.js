...
import { num, compact, pct, days } from './fmt.js';
...
function moveCard(el, meta) {
  ...
  el.innerHTML = `
    ...
    машина могла мати історію й раніше. Воно обрізане і згори: рух, що
    трапився у ${win[win.length - 1]} році, ще не мав шансу розтягнутись так,
    як рух ${win[0]} року. Це й тягне середнє (${days(mig.mean_days_exact)})
    далеко вище медіани (${days(mig.median_days_exact)}): найдовша
    зафіксована пара — ${days(1820)}, і хвіст обірваний межею вікна, а не
    поведінкою власників. Коди операцій не класифікуються взагалі — це
    навмисно, бо високочастотні коди 40, 70/71, 50/80 не піддаються надійній
    класифікації.</p>
    <dl>
